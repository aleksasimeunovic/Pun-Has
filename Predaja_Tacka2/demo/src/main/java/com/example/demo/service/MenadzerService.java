...
public class MenadzerService {
    ...
    public void izmeniPorudzbinu(Menadzer menadzer, UUID uuid, Porudzbina.STATUS status){
        ...
        for(Porudzbina p : obrada){
            if(p.getUUID().equals(uuid)){
                temp = p;
                temp.setStatus(status); // Zasto ga ovde ne promeni?  Vrv je isti razlog i sto u porudzbini pamti samo jedan artikal ....
            }
        }
        /*
       // if("Dostavljena".equalsIgnoreCase(String.valueOf(status))){ // Radi menjanje bodova, ali status idalje ne mrda
            double bodovi = temp.getKupac().getBrojSkupljenihBod();
            temp.getKupac().setBrojSkupljenihBod((bodovi+=temp.getUCena()/1000*133));
