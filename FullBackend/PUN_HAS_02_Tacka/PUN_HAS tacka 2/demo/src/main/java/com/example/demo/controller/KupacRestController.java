...
public class KupacRestController {
    ...
    @PutMapping("/api/kupac/izbaciizkorpe/{uuid}")
    public ResponseEntity<String> izbaci(@PathVariable(name = "uuid")UUID uuid, @RequestBody IdDto idDto, HttpSession session){
        if(!sessionService.validateSession(session)){
            return null;
        }
        Kupac loggedKupac = kupacRepository.getByUsername(sessionService.getUsername(session));

        kupacService.izbaciIzKorpe(loggedKupac, uuid, idDto.getId());
        return ResponseEntity.ok("Izbacen iz korpe artikal sa id: " + idDto.getId());
    }
